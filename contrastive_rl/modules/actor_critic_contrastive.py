...
class ActorCriticContrastive(nn.Module):
    is_recurrent = False
    ...
    def __init__(
        ...
        num_critic_obs,
        num_actions,
        env_vector_dims = 100,  # 对比学习中encoder输出的vector的维度
        actor_hidden_dims=[256, 256, 256],
        critic_hidden_dims=[256, 256, 256],
        encoder_hidden_dims=[256, 256, 256], # 对比学习方法的encoder的hidden层维度
        activation="elu",
        init_noise_std=1.0,
        **kwargs,
    ):
        ...
        mlp_input_dim_a = num_actor_obs + env_vector_dims  # actor输入的维度，除了有观测外，还有barlow twins中的encoder输出的vector
        mlp_input_dim_c = num_critic_obs
        mlp_input_dim_e = num_actor_obs  # encoder的输入维度，和actor的原始观测维度一样
        # Policy
        actor_layers = []
        ...
        for layer_index in range(len(encoder_hidden_dims)):
            if layer_index == len(encoder_hidden_dims) - 1:
                encoder_layers.append(nn.Linear(encoder_hidden_dims[layer_index], env_vector_dims))
            else:
                encoder_layers.append(nn.Linear(encoder_hidden_dims[layer_index], encoder_hidden_dims[layer_index + 1]))
                encoder_layers.append(activation)
        self.source_encoder = nn.Sequential(*encoder_layers)
        self.target_encoder = nn.Sequential(*encoder_layers)

        print(f"Actor MLP: {self.actor}")
        print(f"Critic MLP: {self.critic}")
        print(f"Encoder MLP: {self.source_encoder}")
    ...
    def update_distribution(self, observations): 
        mean = self.actor(torch.cat((observations, self.source_encoder_forward(observations).detach()),dim=-1))  # 根据传入的观测计算actor的实际输出
    ...
    def act_inference(self, observations):  # 计算不添加噪音的actor输出的动作，是用于在inference(训练结束后测试)时使用的
        actions_mean = self.actor(torch.cat((observations, self.source_encoder_forward(observations).detach()),dim=-1))
    ...
    def source_encoder_forward(self, encoder_obs):
        return self.source_encoder(encoder_obs)
    
    def target_encoder_forward(self, encoder_obs):
        return self.target_encoder(encoder_obs)
